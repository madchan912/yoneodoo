...
## 🔮 v5.0 : RAG 고도화 & 비용 최적화
**테마:** "데이터 기반으로 RAG 품질을 높이고 API 비용을 최적화한다"

- [ ] **RAG 조건 확장**: 단백질/지방/나트륨 등 영양소 기반 필터링
  (recipe_nutrition 데이터 활용, min_protein/max_sodium 등)
- [ ] **맛/식감 필터**: 매운맛 제외 등 자연어 조건 → 재료 기반 필터 변환
  (매운 재료 목록 정의: 고추, 고추장, 청양고추 등)
- [ ] **Gemini API 비용 최적화**: rag_search_log 분석 기반
  자주 나오는 조건 패턴 캐싱, 불필요한 호출 줄이기
- [ ] **RAG 품질 평가**: RAGAS 등 평가 프레임워크 도입
  검색 관련도, 답변 충실도 측정
