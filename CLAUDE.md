...
- 아래 변경 발생 시 반드시 같은 커밋에 `CONTEXT.md`도 업데이트할 것:
  - API 엔드포인트 추가/변경
  - CORS 허용 오리진 변경
  - 인프라 변경 (도메인, 포트, 서버 등)
  - 환경변수 추가/변경
...
- **첫 줄**: `[type]: 한 줄 요약`
- **본문**: 배경(왜 필요했는지 자연어로) → 한 일 목록(bullet) → 기대 효과 순서로 작성
- 너무 포멀하지 않게, 나중에 봤을 때 맥락이 바로 파악되는 수준으로

```
feat: AI 기반 재료 정규화 시스템 구축

양파 / 양파한개 / 양파 반개처럼 같은 재료가 여러 형태로 저장되고 있었음
처음엔 손으로 매핑했는데 데이터 늘어나면서 감당이 안됨

- ingredient_mapping 테이블로 raw_name → master_name 정규화 구조 설계
- Gemini API로 미분류 재료 자동 그룹핑·추천
- 어드민이 AI 추천 검토 후 승인하는 Human-in-the-Loop 흐름 구현
