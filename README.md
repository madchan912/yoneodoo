...
>
> **🔗 [요너두 라이브 서비스 보러가기](http://43.201.95.155)**
...
**🔄 v1.9 (예정)**
- 🌐 **커스텀 도메인 + HTTPS**: yoneodoo.kr 연결 및 Let's Encrypt 인증서 적용
...
**🤖 v3.0 (예정)**
- **배치 스케줄러**: 매일 새벽 지정 유튜버 신규 영상 자동 수집·적재
- **클라우드 LLM 파이프라인**: 로컬 Ollama → 서버 내 Gemini API로 완전 이식

**👑 v4.0 (예정)**
- **RAG 기반 AI 식단 플래너**: LangChain + 자체 DB 기반 1주일 식단 자동 큐레이션 (할루시네이션 없는 추천)
- **장바구니 차분 계산**: (식단 필요 재료) - (냉장고 재료) = 이번 주 추가 구매 목록
